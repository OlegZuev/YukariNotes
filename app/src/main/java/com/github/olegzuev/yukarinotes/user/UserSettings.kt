package com.github.olegzuev.yukarinotes.user
...
import com.github.olegzuev.yukarinotes.utils.JsonUtils
...
class UserSettings private constructor(
    ...
    companion object {
        ...
        const val DB_VERSION_JP_NINON = "dbVersion_jp_ninon"
        ...
        const val SERVER_JP_NINON = "jp_ninon"
    ...
    fun getUserServer(): String {
        return preference.getString(SERVER_KEY, SERVER_JP_NINON) ?: SERVER_JP_NINON
    }
    ...
    fun getDbVersion(): Long {
        return when (preference.getString(SERVER_KEY, null)) {
            ...
            SERVER_JP -> {
                ...
                preference.getLong(DB_VERSION_JP_NINON, 0)
            }
        }
    }
    ...
    fun setDbVersion(newVersion: Long, async: Boolean = true) {
        when (preference.getString(SERVER_KEY, SERVER_JP_NINON)) {
            SERVER_JP_NINON -> {
                if (async) {
                    preference.edit().putLong(DB_VERSION_JP_NINON, newVersion).apply()
                } else {
                    preference.edit().putLong(DB_VERSION_JP_NINON, newVersion).commit()
                }
            }
            SERVER_JP -> {
                if (async) {
    ...
    fun getClanBattleLimit(): Boolean {
        return preference.getBoolean(LIMIT_CLAN_BATTLE_KEY, true)
    }
