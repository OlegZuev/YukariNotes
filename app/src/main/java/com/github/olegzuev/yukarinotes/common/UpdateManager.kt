...
import com.github.olegzuev.yukarinotes.utils.BrotliUtils
    ...
    private val mContext: Context)
{
    ...
    private val userAgent = "YukariNotes ${BuildConfig.VERSION_NAME} ${System.getProperty("http.agent")}"
...
//                    iActivityCallBack?.showSnackBar(R.string.db_update_duplicate)
    ...
    fun checkDatabaseVersion() {
        when (UserSettings.get().preference.getString(UserSettings.SERVER_KEY, "jp_ninon")) {
            "jp_ninon" -> {
                checkDatabaseVersionNinon()
            }
            "jp" -> {
                checkDatabaseVersionMain()
            }
            "cn" -> {
                checkDatabaseVersionMain()
            }
    ...
    private fun checkDatabaseVersionMain() {
        val client = OkHttpClient()
...
//                    hasNewVersion = true
    ...
    private fun checkDatabaseVersionNinon() {
        val client = OkHttpClient()
        val request = Request.Builder()
            .url(Statics.LATEST_VERSION_URL)
            .header("User-Agent", userAgent)
